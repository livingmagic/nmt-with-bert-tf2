...
def build_dataset(features, seq_length, batch_size):
...
def from_json_file(bert_config_file):
...
def main(_):
    ...
    # Build model
    model = build_model(bert_config=FLAGS.bert_config_file,
    ...
    with tf.io.gfile.GFile(FLAGS.output_file, "w") as writer:
        for item in dataset:
            unique_ids = list(item["unique_ids"])
            result = model.predict([item["input_ids"], item["input_type_ids"], item["input_mask"]])

            for (i, unique_id) in enumerate(unique_ids):
                line_result = np.squeeze(result[i])
                feature = unique_id_to_feature[unique_id.numpy()]
                output_json = collections.OrderedDict()
                output_json["line"] = int(unique_id.numpy())
                all_features = []
                for (j, token) in enumerate(feature.tokens):
                    features = collections.OrderedDict()
                    features["token"] = token
                    features["feature"] = [
                        round(float(x), 6) for x in line_result[j].flat
                    ]
